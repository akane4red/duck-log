...
let cachedOverview: { at: number; data: ForensicOverview } | null = null;
...
export async function dashboardRoutes(app: FastifyInstance): Promise<void> {
  app.get('/dashboard/overview', async (req, reply) => {
    try {
      ...
      const now = Date.now();
      if (!force && cachedOverview && now - cachedOverview.at < OVERVIEW_CACHE_MS) {
        return reply.send({
          ok: true,
      ...
      const totalsRows = await query<NumericRow>(`
        SELECT
          COUNT(*) AS requests,
      ...
      const statusBreakdown = await query<NumericRow>(`
        SELECT
          status,
          COUNT(*) AS requests
        FROM logs
      ...
      const methodBreakdown = await query<NumericRow>(`
        SELECT
          method,
          COUNT(*) AS requests
        FROM logs
      ...
      const timeline = await query<NumericRow>(`
        WITH hourly AS (
          SELECT
      ...
      const topClientIps = await query<NumericRow>(`
        SELECT
          c_ip,
          COUNT(*) AS requests,
      ...
      const topUris = await query<NumericRow>(`
        SELECT
          uri_stem,
          COUNT(*) AS requests,
      ...
      const suspiciousIps = await query<NumericRow>(`
        SELECT
          c_ip,
          COUNT(*) AS requests,
          COUNT(DISTINCT uri_stem) AS distinct_uris,
