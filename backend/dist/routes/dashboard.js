...
async function dashboardRoutes(app) {
    app.get('/dashboard/overview', async (req, reply) => {
        try {
            ...
            const totalsRows = await (0, connection_1.query)(`
        SELECT
          COUNT(*) AS requests,
        ...
        FROM logs
      `);
            const statusBreakdown = await (0, connection_1.query)(`
        SELECT
          status,
          COUNT(*) AS requests
        FROM logs
        ...
        LIMIT 8
      `);
            const methodBreakdown = await (0, connection_1.query)(`
        SELECT
          method,
          COUNT(*) AS requests
        FROM logs
        ...
        LIMIT 6
      `);
            const timeline = await (0, connection_1.query)(`
        WITH hourly AS (
          SELECT
      ...
      `);
            const topClientIps = await (0, connection_1.query)(`
        SELECT
          c_ip,
          COUNT(*) AS requests,
        ...
        LIMIT 10
      `);
            const topUris = await (0, connection_1.query)(`
        SELECT
          uri_stem,
          COUNT(*) AS requests,
        ...
        LIMIT 10
      `);
            const suspiciousIps = await (0, connection_1.query)(`
        SELECT
          c_ip,
          COUNT(*) AS requests,
          COUNT(DISTINCT uri_stem) AS distinct_uris,
