...
exports.geoIpQuery = void 0;
const connection_1 = require("../db/connection");
const provider_1 = require("../geo/provider");
exports.geoIpQuery = {
    descriptor: {
        name: 'geoip_enrich',
    ...
    async run(params) {
        const manualIps = params.ips ?? null;
        const autoTopN = Math.min(Number(params.auto_top_n ?? 50), 200);
        const flagCountries = new Set((params.flag_countries ?? []).map(c => c.toUpperCase()));
        ...
        if (manualIps && manualIps.length > 0) {
            ...
            const topIps = await (0, connection_1.query)(`
      ...
      `);
        ...
        const geoResults = await provider_1.geoProvider.bulkLookup(ipsToEnrich);
        // Get request counts from logs for context
        const ipList = ipsToEnrich.map(ip => `'${ip}'`).join(',');
        const dateFromClause = dateFrom ? `AND date >= '${dateFrom}'` : '';
        const dateToClause = dateTo ? `AND date <= '${dateTo}'` : '';
        const counts = await (0, connection_1.query)(`
      SELECT
        c_ip,
    ...
    `);
        ...
        // Apply post-filters
        if (datacenterOnly)
            enriched = enriched.filter(r => r.is_datacenter);
        if (torOnly)
            enriched = enriched.filter(r => r.is_tor);
        // Sort: TOR first, then datacenter, then flagged country, then by request count
        enriched.sort((a, b) => {
            if (a.is_tor !== b.is_tor)
                return a.is_tor ? -1 : 1;
            if (a.is_datacenter !== b.is_datacenter)
                return a.is_datacenter ? -1 : 1;
            if (a.is_flagged_country !== b.is_flagged_country)
                return a.is_flagged_country ? -1 : 1;
