    ...
    (async () => {
        ...
        for (const filePath of filePaths) {
            ...
            try {
                ...
                status.processed_files++;
                console.log(`[ingest] ✓ ${status.current_file} → ${result.rows.length.toLocaleString()} rows → ${parquetPath}`);
            }
            catch (err) {
        ...
        console.log(`[ingest] Complete. ${status.total_rows.toLocaleString()} total rows in ${status.duration_ms}ms`);
    })().catch(err => {
        status.running = false;
...
async function registerUnifiedView() {
    ...
    const [count] = await (0, connection_1.query)(`SELECT COUNT(*) as total FROM logs`);
    console.log(`[db] Unified view 'logs' registered — ${count?.total?.toLocaleString()} total rows across ${files.length} file(s)`);
}
